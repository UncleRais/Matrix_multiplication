...
void get_task(int TASK_SCALE, int *ni, int *nj, int *nk, int *nl) {
  switch (TASK_SCALE) {
  case 0:
    *ni = 16;
    *nj = 18;
...
int main(int argc, char** argv)
{
  ...
  for (int TASK_SCALE = 0; TASK_SCALE < 5; TASK_SCALE++) { 
    get_task(TASK_SCALE, &ni, &nj, &nk, &nl);
  ...
  ////////
    ...
    file_printf(file, *message, "Initial / Block: ", times[0] / times[1]);
    file_printf(file, *message, "Block / Parallel_block(for): ", times[1] / times[2]);
    file_printf(file, *message, "Block / Parallel_block(task): ", times[1] / times[3]);

    printf("Initial / Block: %f", times[0] / times[1]);
    printf("\n");
    printf("Block / Parallel_block(for): %f", times[1] / times[2]);
    printf("\n");
    printf("Block / Parallel_block(task): %f", times[1] / times[3]);
    printf("\n\n");
